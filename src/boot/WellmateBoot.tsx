...
const beats = [
  {
    ...
    title: 'Thousands of people die everyday because...',
    tone: 'neutral',
  },
  {
    eyebrow: 'THE FIRST FAILURE',
    title: 'no one called the paramedics...',
    tone: 'red',
  },
  {
    eyebrow: 'THE SECOND FAILURE',
    title: 'the paramedics werent informed fast enough...',
    tone: 'blue',
  },
  {
    eyebrow: 'THE THIRD FAILURE',
    title: 'the paramedics spend too much of precious time testing patient\'s blood sample for correct blood type and allergies...',
    tone: 'clinical',
  },
  {
    eyebrow: 'ENOUGH WAITING',
    title: 'BUT NOT ANYMORE',
    tone: 'impact',
  },
  {
    eyebrow: 'EMERGENCY CARE, REIMAGINED',
    title: 'INTRODUCING WELLMATE',
    tone: 'brand',
  },
]
      ...
      <div className="wellmate-boot-sticky">
        ...
        <div className="wellmate-boot-content">
          {beats.map((beat, index) => {
            ...
            const y = useTransform(scrollYProgress, [start, peak, end], [34, 0, -28])
            const scale = useTransform(scrollYProgress, [start, peak, end], [0.985, 1, 1.01])
            const blur = useTransform(scrollYProgress, [start, peak, end], [6, 0, 5])

            return (
              <motion.div
                key={beat.title}
                className={toneClass(beat.tone)}
                style={{ opacity, y, scale, filter: blur.get() ? `blur(${blur.get()}px)` : undefined }}
              >
        ...
        <button className="wellmate-boot-skip" type="button" onClick={() => setIsFinished(true)}>
          SKIP INTRO
        </button>
